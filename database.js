...
function getNote(id) {
  connection.query("SELECT * FROM notes WHERE note_id=?",[id], function(err, result) {
    if (err) throw err;
    console.log(result);
  });
}
...
function createNotes(mytitle, mycontent) {
      connection.query("INSERT INTO notes (title, contents) VALUES(?, ?)", [mytitle, mycontent], function (err, result) {
        if (err) throw err;
        console.log(result.insertId);
      });
  }
  
  // createNotes("Note 5", "This is note 5 content");
...
function updateNote(mytitle, mycontent, myid) {
    ...
    // console.log(note);
    // if (note){
      connection.query("UPDATE notes SET title=?, contents=? WHERE note_id=?",[mytitle, mycontent, myid], function (err, result) {
      if (err) throw err;
...
function deleteNote(myid) {
  ...
  // console.log(note);
  // if (note){
    connection.query("DELETE FROM notes WHERE note_id=?",[myid], function (err, result) {
    if (err) throw err;
